...
$(document).ready(function(){
	...
	function visual_Slides(_targetWrap, _slides, _has_pause, _type){
		...
		function fade_img(num){
			stop_si();
			$slides.fadeOut(3000);
			$slides.eq(num).fadeIn(3000, function(){
				start_si();
			});
		}
		...
		function start_si(){
			...
			si_01 = setInterval(count_plus, 4000);
		}
		function stop_si(){
			if(si_01 != 0){
				clearInterval(si_01)
	...
	var slide_01 = visual_Slides(".image_box", ".slides", false, "slide");//맨 마지막에 호출을 한다 mobile, tablet, desktop 다 사용하려고
